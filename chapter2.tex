...
This chapter presents the state-of-the-art in Analog-to-Digital Converters, reviewing theoretical foundations and analysing survey data to justify the proposed architecture.
...
The figures of merit defined above do not transfer directly to event-driven converters. In a level-crossing architecture, whether continuous- or discrete-time, the sampling rate is not a design constant: it depends on the activity and slope of the input signal. A formulation built around a fixed Nyquist frequency $f_s$ therefore misrepresents the efficiency of these topologies.

The Walden FoM assumes a time-invariant sampling frequency. Level-crossing quantisers, as Weltin-Wu and Tsividis demonstrated, generate a sample only when the input crosses a quantisation threshold \cite{weltin2013}. When the input is sparse, few redundant samples are produced, and the saving carries through the whole downstream digital processing chain. The absence of a uniform sampling clock also changes the noise spectrum: the output is free of aliasing and of the usual quantisation noise floor, and under some signal conditions the SNDR can exceed the theoretical limit of a conventional converter with the same nominal number of bits.

A fair evaluation therefore replaces the fixed $f_s$ in the Walden equation with the average rate of actual conversion events per unit time, $f_{s,avg}$. The discrete-time level-crossing ADC of Mou \textit{et al.}\ shows why this matters: its power consumption varies between $95\,\mathrm{nW}$ for highly time-sparse stimuli, such as neural signals, and $250\,\mathrm{nW}$ for generic waveforms, so no single static number describes its efficiency \cite{mou2026}.

With this adaptation, energy efficiency is reported in femtojoules per actual conversion step, and as a range rather than a single value. The prototype of Mou \textit{et al.}\ spans $5.3$ to $14.2\,\mathrm{fJ}$ per conversion step depending on the transition pattern of the input.

The variation has a physical origin. The amplitude change between successive samples, $\Delta V_{\mathrm{in}}$, determines how much work the converter must do. Slow events, with steps between one and two LSB, require only a linear $\pm1\,\mathrm{LSB}$ update of the feedback loop and few comparisons. Abrupt variations beyond two LSB trigger a binary search similar to a SAR conversion, with more comparisons and more energy per event.

Throughout this work, the figures of merit of asynchronous converters are therefore indexed to $f_{s,avg}$, so that signal sparsity appears as a genuine efficiency gain instead of being hidden by uniform-sampling assumptions.
...
Compared to dynamic offset-cancellation techniques such as auto-zeroing, which rely on accurately timed clock phases $\phi_1$ $\phi_2$ and additional storage capacitors that must be periodically charged and discharged \cite{baker_cmos_2010}, static trimming techniques — whether implemented through resistive networks or through bulk-driven threshold adjustment as in \cite{yao_bulk_2011} — are particularly well suited to an event-driven asynchronous architecture. Once the correction values are determined during a dedicated foreground calibration phase, the trimming network remains static, meaning that no further switching activity or clocking is required and the comparator bias conditions are held purely by DC conduction paths. This static behaviour ensures that the trimming circuitry does not disturb the intrinsic event-driven operation of the ADC, which should ideally remain idle whenever the input signal is not crossing a quantisation threshold.

Another important advantage, particularly pronounced in the bulk-driven variant, is that the correction acts on terminals that are not part of the signal path: driving the bulk of the comparison devices shifts their threshold voltage through the body effect without placing additional capacitances on the high-speed nodes, thereby preserving the original small-signal bandwidth and regeneration speed. In contrast, dynamic calibration schemes typically introduce extra sampling capacitors and switch parasitics at sensitive nodes, which can degrade the comparator’s response time and limit the maximum achievable sampling rate in high-speed Flash architectures \cite{baker_cmos_2010}. These properties motivate the static, bulk-driven trimming strategy adopted in Chapter~\ref{chap:trimming}, where the body bias of the comparison stage is used to correct deterministic trip-point deviations across process corners.
\newpage
...
The impact of this architectural difference on long-term technology trends is illustrated in Figure \ref{fig:energy_year}. The plot depicts the energy per conversion ($P/f_s$) over the last two decades. While both paradigms show a downward trend due to process node scaling, asynchronous designs consistently occupy the lower bound of the energy envelope. This suggests that eliminating the global clock is a key enabler for breaking the power efficiency barriers in modern designs.
...
\end{tabular}
\end{table}
From the previous analysis, it becomes clear that synchronous architectures face a fundamental inefficiency when dealing with time-sparse sensor signals: the global clock forces continuous switching activity, so dynamic power remains proportional to the clock frequency even during long periods with no new information content. In contrast, event-driven schemes naturally decouple power consumption from the Nyquist rate and tie it instead to the actual rate of relevant signal transitions. Combining this observation with the survey data on state-of-the-art converters, where Flash ADCs define the speed frontier but suffer from poor Walden FoM at high sampling rates, while more energy-efficient SAR and sigma-delta architectures are optimised for lower bandwidths, motivates the choice of an asynchronous Flash topology as the core of this work. By removing the global sampling clock and operating comparators in a level-crossing, event-driven manner, the proposed asynchronous Flash ADC aims to preserve the intrinsic low-latency and high-speed advantages of the Flash architecture, while leveraging signal sparsity to significantly improve energy efficiency and close the FoM gap with the best-performing synchronous designs in the targeted operating region.
...
Regarding the implementation of those comparison elements, the current-mode techniques reviewed in Section~\ref{sec:clockless_event} constituted the starting point of the design: processing the input as a current promises fast, clockless decisions, and the study of current comparator topologies informed the first architectural phase explored in Chapter~\ref{chap:async}. However, as that exploration will show, the cumulative voltage headroom demanded by the current-mode structures proved incompatible with the $0.8$~V supply of the target technology. The final architecture therefore operates in voltage mode, combining a unity-gain voltage follower with a bank of ratioed CMOS slicers whose switching thresholds are set by transistor sizing. The literature reviewed in this chapter thus serves a dual purpose: the current-mode material documents the origin and the rejection criteria of the first design phase, while the trimming techniques of Section~\ref{sec:calibration} provide the foundation for the body bias correction strategy developed in Chapter~\ref{chap:trimming}.
