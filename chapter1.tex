...
\end{figure}
The time-sparse nature of many signals can be observed in the burst waveform where information is concentrated in short intervals separated by long idle periods, as shown in Fig.~\ref{fig:Sparse-time}. In particular, subfigures (b)–(d) highlight how continuous-time and discrete-time event-driven strategies avoid generating samples during inactivity, in contrast with conventional uniform sampling which keeps operating at a fixed Nyquist rate regardless of the signal content, as shown in Fig.~\ref{fig:Sparse-time}.
...
\end{figure}
The impact of device mismatch on the ADC transfer characteristic is illustrated by the deviation between the ideal and actual staircase curves, as shown in Fig.~\ref{fig:Offset-error}. In this figure, varying step widths and the presence of a missing output code clearly demonstrate how Differential Non-Linearity (DNL) errors distort the ideal 1~LSB step profile, as shown in Fig.~\ref{fig:Offset-error}.
...
\caption[SNDR versus sampling frequency for representative ADCs]{SNDR vs. Sampling Frequency (Flash reference [1] highlighted)}
\label{fig:snr_fs}
\end{figure}
The dependence of SNDR on the sampling frequency for several state-of-the-art designs is presented in Fig.~\ref{fig:snr_fs}. By comparing the trajectories of different architectures, the figure shows how high-resolution sigma-delta converters sustain very high SNDR at low $f_s$, whereas Flash and SAR topologies occupy regions optimized for speed or moderate resolution, as shown in Fig.~\ref{fig:snr_fs}.
\par\vspace{1.5em}
\par\vspace{1.5em}
\par\vspace{1.5em}
...
\paragraph{}
\paragraph{}
The dual-slope architecture converts the input voltage into time by integrating it during a fixed interval and then discharging the capacitor with a known reference, as shown in Fig.~\ref{fig:dual_slope_arch}. The digital counter measures the discharge interval controlled by the reference voltage, so the resulting count is proportional to the average value of the input signal and inherently rejects high-frequency noise, as shown in Fig.~\ref{fig:dual_slope_arch}.
\newpage
...
random process variations, as shown in Fig.~\ref{fig:mismatch_correction} and Fig.~\ref{fig:resistive_ladder} \cite{yaobulk2011}.
...
\par\vspace{1.5em}
\par\vspace{1.5em}
The long-term reduction in energy per conversion step driven by architectural innovation and technology scaling is depicted in Fig.~\ref{fig:energy_year}. Each point in the plot corresponds to a published design, making it possible to compare how different ADC families populate the energy–year plane and to identify the most efficient solutions, as shown in Fig.~\ref{fig:energy_year}.
\begin{table}[htbp]
\centering
