...
Smart sensors are now everywhere, from wearable health monitors to distributed environmental and industrial nodes. The processing and storage of information are digital and have benefited from the scaling described by Moore's law, but the physical world is analog, temperature, sound and pressure are continuous in both time and amplitude. The Analog-to-Digital Converter (ADC) is the interface between the two domains, and every digital system that measures the real world depends on it \cite{johns_martin_2012}.

Energy efficiency has moved from a secondary performance metric to a primary design constraint. Many sensors run on small batteries or on harvested energy, so every joule dissipated shortens the autonomy of the system.
...
% Therefore, combining the high speed of the Flash architecture with the high power efficiency required for smart sensors demands a radical shift away from the synchronous clocking scheme.
