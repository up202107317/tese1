...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{figures/offset_error.png}
  \caption{Input offset voltage ($V_{os}$) generation in CMOS comparators due to transistor mismatch. The figure illustrates how random variations in threshold voltage ($V_{th}$) and gain factor ($\beta$) in the differential input pair result in an offset voltage that shifts the trip point of the comparator \cite{johns_martin_2012}. This mismatch-induced offset becomes the primary limitation for converter accuracy in deep-submicron technologies.}
...
The performance benchmarks and comparative trends presented in this section were derived from a comprehensive survey of the IEEE International Solid-State Circuits Conference (ISSCC) data from 1997 to 2022. The ISSCC was chosen as the primary data source because it is widely regarded as the premier forum for solid-state circuit design, requiring experimental silicon-measured results for every publication. This ensures that the comparison is based on proven hardware rather than theoretical simulations.

The analytical plots and tables presented below incorporate data from an exhaustive list of state-of-the-art works. The comparative baseline for Flash architectures is established by \cite{Chen_2006, Lukas_2014, Draxelmayr_2014}. The boundaries for energy efficiency and high-resolution performance (the "survey envelopes") are defined by the record-holders described in \cite{Jiang_2018, Veldhoven_2021, Ginsburg_2014, Kim_2018, Jang_2018, De_Cano_Garcia_2019, Muller_2015} for power efficiency, and \cite{Delic-Ibus_2013, Jang_2018_2, Abe_2016, Geelen_1997} for high resolution and dynamic range. Collectively, these references \cite{Geelen_1997, Chen_2006, Delic-Ibus_2013, Ginsburg_2014, Lukas_2014, Draxelmayr_2014, Muller_2015, Abe_2016, Kim_2018, Jang_2018, Jang_2018_2, Jiang_2018, De_Cano_Garcia_2019, Veldhoven_2021} represent the performance frontier across all major ADC architectures.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/Delta-sigma 1st order.png}
  \caption{First-order Sigma-Delta modulator architecture showing the integrator, quantizer, and feedback path. The modulator operates at a clock frequency much higher than the Nyquist rate, and the quantization noise is shaped to frequencies outside the signal band, allowing digital filtering to recover high-resolution signals \cite{candy_1985}.}
  \label{fig:sd_arch}
\end{figure}
...
% [ATENÇÃO]: As seguintes referências NÃO estão no references.bib
% Opções: (1) Adicionar ao .bib, (2) Remover citações, (3) Substituir por refs equivalentes
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/Energy evo.png} 
  \caption{Energy per conversion evolution ($P/f_s$) over the years, highlighting the lower energy floor achieved by asynchronous designs. The triangular markers represent asynchronous ADC implementations, while circular markers represent synchronous designs. The plot demonstrates that asynchronous architectures consistently achieve lower energy consumption across multiple technology nodes compared to their synchronous counterparts, validating the energy efficiency advantage of event-driven operation. Data compiled from ISSCC publications (1997-2025) \cite{lim_2013,Tamba,Schvan_2006,Hammerschmied,Vorenkamp,Yong_In_Park,Ingino,Yoshioka_2010,Liu_2010}.}
...
% O texto acima deve ser inserido aqui.

\begin{figure}[htbp]
\centering
\resizebox{\textwidth}{!}{ % Garante o alinhamento correto com as margens da página
\begin{ganttchart}[
    hgrid,
    vgrid,
    x unit=0.5cm,
...
]{1}{24} % Representa as semanas de fevereiro a julho
    \gantttitle{2026}{24} \\
    \gantttitle{Feb}{2} 
    \gantttitle{Mar}{4} 
    \gantttitle{Apr}{4} 
    ...
    % Atribuição de nomes (name=...) para evitar erros de compilação nos links
    \ganttbar[name=T1]{Mathematical Formulation (Ch. 3)}{1}{4} \\
    \ganttbar[name=T2]{ADC Design \& Simulation}{4}{12} \\
    \ganttbar[name=T3]{Results Analysis (Ch. 4)}{12}{18} \\
    \ganttbar[name=T4]{Scientific Article Preparation}{14}{20} \\
    \ganttbar[name=T5]{Dissertation Final Writing}{16}{19} \\
    \ganttmilestone[name=M1]{Provisional Delivery (Jun 30)}{19} \\
    \ganttbar[name=T6]{Defense Preparation}{20}{22} \\
    \ganttmilestone[name=M2]{Thesis Defense (Jul 24)}{23} \\
    \ganttmilestone[name=M3]{Final Submission (Jul 31)}{24}

    % Links utilizando os nomes definidos
    \ganttlink{T1}{T2}
    \ganttlink{T2}{T3}
    \ganttlink{T5}{M1}
\end{ganttchart}
}
\caption{Work plan for the second semester of the dissertation.}
...
For the development and verification of the proposed asynchronous Flash ADC, the Synopsys software suite will be used as the primary tool for circuit-level design and simulation. This professional toolset allows for precise transistor-level modeling and the verification of the offset trimming mechanism through comprehensive analog and mixed-signal simulations. Specifically, Synopsys tools will be utilized to model the threshold voltage variations in the differential pair and to perform Monte Carlo analysis to assess the impact of process mismatches. The simulations and data processing will be conducted on a personal computer equipped with an Intel Core i7 processor and 16GB of RAM, which provides the computational capacity required for these tasks. Since the university provides access to the necessary Synopsys licenses, no additional costs are anticipated for the institution or the author during the development of this work.